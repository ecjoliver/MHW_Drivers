...
for i in $(seq 0 30 350);
  ...
  do
     ...
     fo_out=mhw_data_98pc/mhw_severity.pc98.$stem
     if [ -f $fo ]; then	
     if [ ! -f $fo_out ]; then
        # RUN AS PARALLEL JOBS
        echo processing $fo_out .....
    	JOB=`qsub  -q express -l walltime=1:00:00,mem=8000MB  -N m98_$i_$(($j-90)) - << EOJ
#PBS -S /bin/bash
cd /g/data1a/e14/asg561/MHW
module use ~access/modules
module load pythonlib/numpy/1.9.3
module load pythonlib/netCDF4/1.0.4
python regional_MHW_pc98_reducedFileSize.py $fo 
